...
switch ($_GET["op"]) {
    ...
    $cod_persona=$_REQUEST["idcliente"]; 
		$rspta=$asistencia->listar_asistencia($fecha_inicio,$fecha_fin,$cod_persona);
		//declaramos un array
		$data=Array();
	...
	break;
		...
		case 'selectPersona':
			...
			while ($reg=$rspta->fetch_object()) {
				echo '<option value=' . $reg->cod_persona.'>'.$reg->nombre.' '.$reg->apellidos.'</option>';
			}
			break;
