...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		...
		if (empty($idusuario)) {
			...
			$rspta=$usuario->insertar($nombre,$apellidos,$login,$iddepartamento,$idtipousuario,$email,$clavehash,$imagen,$usuariocreado,$cod_persona);
			echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar todos los datos del usuario";
		}
		else {
			$rspta=$usuario->editar($idusuario,$nombre,$apellidos,$login,$iddepartamento,$idtipousuario,$email,$imagen,$usuariocreado,$cod_persona);
...
//case para verigficar el login 
	case 'verificar':
		...
		$clavehash=hash("SHA256", $claveadmin);
	
		$rspta=$usuario->verificar($logAdmin, $clavehash);
		...
		if (isset($fetch)) 
		{
			...
			$_SESSION['cod_persona']=$fetch->cod_persona;
	...
	break;
// caso para que el usuario salga
	case 'salir':
